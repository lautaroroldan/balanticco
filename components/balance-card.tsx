...
async function BalanceCard({ className, userId }: BalanceCardProps) {
    const currentDate = new Date()
    const month = currentDate.getMonth() + 1
    const year = currentDate.getFullYear()

    const { totalExpense, totalIncome, balance, firstTransferDate: initialDate, lastTransferDate: finalDate } = await fetchBalanceCardData(userId, month, year)
